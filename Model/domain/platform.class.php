...
class Platform extends Model{
    ...
    public function checkOlddomainInfo($domain){
        return $this->table(self::__OLDDUBDOMAIN_TABLE__)->where(['M_account_domain' => $domain])->find();
    }
   
    public function checknewddomainInfo($newsql,$domain){
        return $newsql->table(self::__DUBDOMAIN_TABLE__)->where(['p_account_domain' => $domain])->find();
    }
  
    public function checknewdAgain($newsql,$domain){
        return $newsql->table(self::__DUBDOMAIN_TABLE__)->where(['p_domain' => $domain])->find();
    }
    
    public function checkolddAgain($domain){
